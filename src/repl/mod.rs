...
use crate::{vm::VM, assembler::instruction_parsers::program};
    ...
    pub fn run(&mut self) -> () {
        ...
        loop {
            ...
            match buffer {
                ...
                _ => {
                    let (_, parsed_program) = program(CompleteStr(buffer)).unwrap();
                    println!("{:#?}", parsed_program.to_bytes());
                    for byte in parsed_program.to_bytes() {
